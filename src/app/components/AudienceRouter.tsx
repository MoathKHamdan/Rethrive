...
const audiences = [
  ...
  },
  {
    ...
    icon: Users,
    color: "bg-brand-900", // Dark neutral
    hoverBorder: "hover:border-brand-900",
        ...
        >
            {audiences.map((item, idx) => (
                <motion.div
                    key={idx}
                    variants={fadeInUp}
                    className={`relative p-8 rounded-3xl bg-brand-50 border-2 border-transparent ${item.hoverBorder} transition-all duration-300 group hover:bg-white hover:shadow-xl flex flex-col`}
                >
                    ...
                    <p className="text-brand-600 mb-8 leading-relaxed flex-grow">
                        {item.description}
                    </p>
                    
                    <a href="#contact" className={`inline-flex items-center gap-2 font-bold transition-colors ${item.color === 'bg-[#187F7E]' ? 'text-[#187F7E]' : 'text-brand-900'}`}>
